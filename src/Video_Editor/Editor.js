...
class Editor extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            timings: [],
            playing: false,
            currently_grabbed: {"index": -1, "type": "none"},
            difference: 0.2,
    ...
    componentDidMount = () => {
        ...
        this.playVideo.current.addEventListener('timeupdate', function () {
            var seek = (self.playVideo.current.currentTime - self.state.timings[(self.state.currently_grabbed.index != -1) ? self.state.currently_grabbed.index : 0].start) / self.playVideo.current.duration * 100;
            //todo
            self.progressBar.current.style.width = `${seek}%`;
            if (self.playVideo.current.currentTime >= self.playVideo.current.duration) {
    ...
    seekVideo = (seekRatio) => {
        this.playVideo.current.currentTime = this.playVideo.current.duration * seekRatio
    }
            ...
            <div key={"grabber_"+index}>
                <div className="grabber start" style={{left: `${x.start / this.playVideo.current.duration * 100}%`}} onMouseDown={(event) => {
                    if(this.state.deletingGrabber){
                        ...
                        window.addEventListener('mousemove', this.startGrabberMove); this.setState({currently_grabbed: {"index": index, "type": "start"}})
                    }
                }}>
                ...
                <div className="grabber end" style={{left: `${x.end / this.playVideo.current.duration * 100}%`}} onMouseDown={(event) => {
                    if(this.state.deletingGrabber){
                        ...
                        window.addEventListener('mousemove', this.startGrabberMove); this.setState({currently_grabbed: {"index": index, "type": "end"}})
                    }
                }}>
                    <svg version="1.1" xmlns="http://www.w3.org/2000/svg" x="0" y="0" width="10" height="14" viewBox="0 0 10 14" xmlSpace="preserve">
                        <path className="st0" d="M1 14L1 14c-0.6 0-1-0.4-1-1V1c0-0.6 0.4-1 1-1h0c0.6 0 1 0.4 1 1v12C2 13.6 1.6 14 1 14zM5 14L5 14c-0.6 0-1-0.4-1-1V1c0-0.6 0.4-1 1-1h0c0.6 0 1 0.4 1 1v12C6 13.6 5.6 14 5 14zM9 14L9 14c-0.6 0-1-0.4-1-1V1c0-0.6 0.4-1 1-1h0c0.6 0 1 0.4 1 1v12C10 13.6 9.6 14 9 14z"
    ...
    deleteGrabber = (index) => {
        ...
        time.splice(index, 1);
        this.setState({timings: time, deletingGrabber: false, current_warning: null})
    }
        ...
        return(
            <div className="wrapper">
                ...
                <div className="controls">
                    ...
                    <div className="player-controls">
                        <button className="seek-start" title="Skip to Start" onClick={() => {this.playVideo.current.currentTime = this.state.timings[0].start}}><FontAwesomeIcon icon={faStepBackward} /></button>
                        ...
                        <button className="seek-end" title="Skip to End" onClick={() => {this.playVideo.current.currentTime = this.state.timings[this.state.timings.length-1].end}}><FontAwesomeIcon icon={faStepForward} /></button>
                    </div>
                    <div>
